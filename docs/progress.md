...
- 07-31 第二次实战：Qwen2.5-VL-7B 候选 -> 家长修正（过度合并 195min/106min Focus、
  ...
  7 条 parent feedback。
- 08-01 实战进一步确认边界规则：30 分钟 Focus 正好达标，30 分钟 Screen 正好
  不扣分；成人经过不等于 Coaching；成人独处、离开重入和任务重置均不得被短
  过渡规则桥接。Skill directive 已升级为 v5.6，并加入全覆盖粗查、边界加密复核、
  超长阶段/低光身份复核。
- 本机 ready 触发器默认按最近 6 次逐日完成时间在 22:30–00:00 轮询，候选成功、
  未 ready 或分析失败均形成 Pushover 闭环；同一视频已有 matching
...
2. 连续观察每日 22:30–00:00 自动触发与 Pushover 候选通知，按真实耗时调整窗口；
