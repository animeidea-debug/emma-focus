...
docker exec -e YINGSHI_RES="$YINGSHI_RES" -e MAX_LOG_SIZE="$MAX_LOG_SIZE" -e TEST_MODE="$TEST_MODE" -e CAMERA_NAME="yingshi" tdarr_node sh -c '
    ...
    # 提取最新日期路径（用于跳过未录完的不完整日期）
    LATEST_DATE_YS=$(tail -1 /tmp/ys_dates.txt)
    ...
    while read -r d_path; do
        ...
        if [ "$d_path" = "$LATEST_DATE_YS" ] && [ "$TEST_MODE" != "true" ]; then
            HAS_AFTERNOON=$(find "$d_path" -maxdepth 1 -type d -name "13" -o -name "14" -o -name "15" -o -name "16" -o -name "17" -o -name "18" -o -name "19" -o -name "20" -o -name "21" -o -name "22" 2>/dev/null | head -1)
            if [ -z "$HAS_AFTERNOON" ]; then
                echo "ℹ️ 最新日期 ${DATE_STR} 尚无下午录像（13:00-22:59），跳过（待明日补全）。" >> "$LOG_FILE"
                continue
            fi
        fi
        ...
        while read -r hour_dir; do
            # ⚠️ find 会把 d_path 自身也匹配（目录名如 09 符合 [0-9][0-9]）
            if [ "$hour_dir" = "$d_path" ]; then continue; fi
            ...
            # 收集当前小时内的所有原始视频片段
            find "$hour_dir" -type f -name "*.mp4" | sort | while IFS= read -r f; do printf "file '%s'\n" "$f"; done > "$WORK_TMP/list_ys_${DATE_STR}_${HOUR_VAL}.txt"
