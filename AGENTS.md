...
Use these document roles consistently:

- `AGENTS.md`: durable instructions for coding agents—repository boundaries, invariants, safety rules, validation, and workflow. Do not use it for dated status, session notes, or a backlog.
- `README.md`: stable human-facing overview—product purpose, architecture, setup, common operations, and links to deeper documentation.
- `docs/progress.md`: current handoff snapshot—last verified date, deployed and repository state, active work, known issues, and ordered next steps. Keep it concise and replace stale status instead of accumulating a chronological diary.
- `docs/decisions/`: durable architecture decision records for choices that need rationale and consequences.
- `docs/runbooks/`: repeatable operational and recovery procedures.
- `docs/archive/`: superseded plans and historical material that remain useful for reference.

The filename is `AGENTS.md` (plural). Keep credentials, local key paths, personal data, and machine-specific secret configuration out of all committed documentation.
